...
channelRoute.post("/", Auth.authorize, (req: Request, res: Response) => {
	if(req.body.channel.owners)
		if(!req.body.channel.owners.includes(req.user!._id)) req.body.channel.owners.push(req.user!._id);
	else req.body.channel.owners=[req.user!._id];
