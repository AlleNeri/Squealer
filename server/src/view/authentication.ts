				...
				if(!user) res.status(500).json({ success: false, msg: "Error creating user. It's probably a server error." });
					...
					Auth.signInWithUser(user, req.body.password)
						.then((token: any | null)=> {
							...
							else res.status(201).json({ success: true, msg: "Successful created new user.", id: token.userId, jwt: token.authToken, userType: token.userType });
						})
...
authenticationRoute.post("/login", (req: Request, res: Response) => {
	...
	//if yes, find the user
		...
		Auth.signIn(req.body.username, req.body.password)
			.then((token: any | null)=> {
				if(!token) res.status(500).json({ success: false, msg: "Error accessing user." });
				else if(token.block) res.status(401).json({ success: false, msg: "User is blocked.", id: token.userId, userType: token.userType, block: true });
